...
%             Mahr_rep = cell2mat(cellfun(@(x) reshape(-sqrt(mahal(NTRmat,x)'),1,1,T,P),DIRREG,'Uni',0));
    ...
    %             te(cnt) = toc;
        ...
        MD_d = nanmean(MD_dboot,4);
        MD_t = nanmean(MD_tboot,3);
%         for i = 1:P
%             
%             tic; 
%             cnt = cnt + 1;
...
%             te(cnt) = toc;
%         end
        M.([lower(area) '_boot']){blck}.d = MD_d;
        M.([lower(area) '_boot']){blck}.t = MD_t;
%         M.([lower(area) '_boot']){blck}.ep = MD_ep;
        
        clc; fprintf('trial block %d/%d\n',blck,length(Project_blocks));

    end
    te = toc;
    ms = sec2min(te); 
    clc; fprintf('run took %d:%d%d\n',ms(1),ms(2),ms(3));
