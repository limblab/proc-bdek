...
%     roi = mat2cell([(1:(CLENS(end)))',(1:CLENS(end))'],ones(CLENS(end),1),2);
    ...
    for blck = 1:length(Project_blocks)
        ...
        for i = 1:P
            ...
            NTR_rep = repmat(NTR,nb,1) + repmat(NTRv,nb,1).*randn(size(NTR,1)*nb,NDIM);

            Mahr_rep = cell2mat(cellfun(@(x) reshape(-sqrt(mahal(NTR_rep,x)),1,1,T,nb),DIRREG,'Uni',0));
     
%             Mahr = permute(reshape(cell2mat(cellfun(@(x) -sqrt(mahal(NTR,x)'),DIRREG,'Uni',0)),8,T,[]),[1 3 2]);
            ...
            for t = 1:T
                ...
                MD_bt = zeros(8,nb);
                for btlp = 1:nb
                    Maldists = Mahr_rep(:,:,t,btlp);
                    [vl,loc] = max(max(Maldists,[],1));

                    diratmax = nanmean(Maldists(:,loc),2);

                    if isnan(vl)
                        MT_bt(btlp) = NaN;
                    else
                        MT_bt(btlp) = roicents(loc);
                    end
  
                    MD_bt(:,btlp) = diratmax;
                end
                
                MD_t(i,t) = nanmean(MT_bt);
                MD_d(:,t,i) = circshift(nanmean(MD_bt,2),5-targids{bli}(i));
...
%                 end
        ...
        M.([lower(area) 'bt']){bli}.d = MD_d;
        M.([lower(area) 'bt']){bli}.t = MD_t;
    end
end
