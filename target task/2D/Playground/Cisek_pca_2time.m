...
%               [20 -1000] , [20   0]   ;...
    ...
    execution_index = autosplit{sum(cell2mat(cellfun(@(x) x==[20 0],alcols(:,1),'Uni',0)),2)==2}(1);
...
%         MD_tboot = zeros(P,T,NBOOT);
        [MD_dboot,MEX_dboot,MD_dboot1,MD_dboot2] = deal(zeros(length(targs),T,P,NBOOT));
        ...
        for bootnum = 1:NBOOT    
            ...
            DR_ball2 = cell(8,1);
            for tlc = 1:8
                DR_ball1{tlc,1} = cell2mat(DR_b(tlc,delay)');
                DR_ball2{tlc,1} = cell2mat(DR_b(tlc,move)');
            end
...
%                     Mahr = cell2mat(cellfun(@(x) 1./sqrt(mahal(NTR(:,:,t),x)'),DR_b(:,t),'Uni',0));
                Mahr1 = cell2mat(cellfun(@(x) 1./sqrt(mahal(NTR(:,:,t),x)'),DR_ball1,'Uni',0));
                Mahr2 = cell2mat(cellfun(@(x) 1./sqrt(mahal(NTR(:,:,t),x)'),DR_ball2,'Uni',0));
%                 Mahr_ex = cell2mat(cellfun(@(x) 1./sqrt(mahal(NTR(:,:,t),x)'),DR_ex,'Uni',0));
                for i = 1:size(Mahr1,2)
%                     MD_dboot(:,t,i,bootnum) = circshift(Mahr(:,i),5-targids{bli}(i));
%                     MD_dboot3(:,t,i,bootnum) = circshift(Mahr3(:,i),5-targids{bli}(i));
                    MD_dboot1(:,t,i,bootnum) = circshift(Mahr1(:,i),5-targids{bli}(i));
                    MD_dboot2(:,t,i,bootnum) = circshift(Mahr2(:,i),5-targids{bli}(i));
%                     MEX_dboot(:,t,i,bootnum) = circshift(Mahr_ex(:,i),5-targids{bli}(i));
        ...
        M.([lower(area) '1']){bli}.d = nanmean(MD_dboot1,4);
        M.([lower(area) '2']){bli}.d = nanmean(MD_dboot2,4);
