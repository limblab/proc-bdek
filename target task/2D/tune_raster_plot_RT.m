...
[~,~,~,~,~,~,dirrast_RT] = co_tuning(areaunits,combtt(:,10),combtt,t3,t4,12,unitidx);
...
for i = 1:(length(tune_times))
    ...
    for j = 1:size(dirrastSUBTIME{i},1)
        for k = 1:size(dirrastSUBTIME{i},2)
            if size(dirrastSUBTIME{i}{j,k},1)<=triallim
                dirrastSUBTIME{i}{j,k} = [];
            end
        end
    end
...
for i = 1:8
    ...
    for j = 1:size(dirrast,2)
        ...
        currast_RT = dirrast_RT{i,j};
        currast_RT(currast_RT==0)=nan;
        
%         xsRT = round(1000*(combtt(currast_inds,12)-combtt(currast_inds,5)))+t3;
%         xsRTgo = round(1000*(combtt(currast_inds,6)-combtt(currast_inds,5)))+t3;
...
%         [~,gocuesort] = sortrows(xsRTgo');
        ...
        for k = 1:size(currast,1)
            ...
            currast(:,(t2+t3-100):end) = [];
            xs = find(currast(k,:)==1); % 
            
            xsRT = find(currast_RT(k,:)==1)+t2+t3-t1;

            ys = find(gocuesort==k);
   
            if size(currast,1)>triallim
                plottick(xarray(xs),ys*ones(size(xs)),size(currast,1),maxes+2,j,colplot{j});
                plottick(xarray(xsRT),ys*ones(size(xsRT)),size(currast,1),maxes+2,j,colplot{j});
                
                plottick(xsgocue(k),ys,size(currast,1),maxes+2,j,'c');
                plottick(t2,ys,size(currast,1),maxes+2,j,'m');
...
%                 plottick(xsendtrial(k),ys,size(currast,1),maxes+2,j,'y');
        ...
        plot([t1,t2+t4],nansum(indoffset(1:j))*[1 1],'k-');
        xlim([t1 t2+t4]);
        ylim([0 3*maxes]);
    end
end
