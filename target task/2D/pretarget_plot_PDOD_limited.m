...
% loop_ranges = {[-200:25:700],[-200:25:100]};%,[-100:100:400]};
loop_ranges = {[-200 0]};
% loop_ranges = {[500 700]};
% timelimiter = {inf(size(alldays(2).tt,1),1), alldays(2).tt(:,7)};
timelimiter = {alldays(2).tt(:,7)};
...
reprch = repmat(alldays(prediction_day).tt(:,reachdir_col),1,size(reppd,2)); USE THIS
...
if do_expected
    ...
    [expect_counts] = deal(cell(length(loop_alignments),1));
...
for loopthrough = 1:length(loop_alignments)
    ...
    for q = 1:length(co_units)
        ...
        if 1%~isnan(best_PDS(q))
            out = raster_get(co_units{q},tt_pred,time_bins([1,end])./1000,time_align,timelimiter{loopthrough});
    %         out(isnan(out))=0;

            rast = zeros(size(out,1),length(time_bins)-1);
            binsizes = time_bins - time_bins(1); binsizes(1) = 0;
            for v = 1:length(time_bins)-1
                rast(:,v) = nanmean(out(:,(binsizes(v)+1):binsizes(v+1)),2)*1000;
    %             rast(:,v) = sum(out(:,(binsizes(v)+1):binsizes(v+1)),2);
            end
            unit_counts{loopthrough}{q} = rast;% FR ./repmat(diff(time_bins)/1000,size(rast,1),1); % in FIRING RATE
        else
            unit_counts{loopthrough}{q} = NaN(size(tt_pred,1),length(time_bins)-1);
        end
    end
