...
%uc = circ_var(alldays(pdi).slices')';
...
figure; hold on; xs = (time_bins(1:(end-1)) +time_bins(2:end))/2;
for j = 1:size(FD,2)
    clc; fprintf('%d/%d\n',j,size(FD,2));
    y = FD(:,j);
...
%     
    fitimproveLH_unc(j,1) = mean(fitimprove_unc(j,:)); fitimproveLH_unc(j,2) = std(fitimprove_unc(j,:));
    fitimproveLH_dfp(j,1) = mean(fitimprove_dfp(j,:)); fitimproveLH_dfp(j,2) = std(fitimprove_dfp(j,:));
%     
...
%%
plot(xs,fitimproveLH_unc(:,1),'c');
patch([xs fliplr(xs)],...
    [fitimproveLH_unc(:,1)'+fitimproveLH_unc(:,2)', fliplr(fitimproveLH_unc(:,1)'-fitimproveLH_unc(:,2)')],'c','FaceAlpha',0.25);

plot(xs,fitimproveLH_dfp(:,1),'g'); 
patch([xs fliplr(xs)],...
    [fitimproveLH_dfp(:,1)'+fitimproveLH_dfp(:,2)', fliplr(fitimproveLH_dfp(:,1)'-fitimproveLH_dfp(:,2)')],'g','FaceAlpha',0.25);
% %%
% for i = 1:length(LI)
%     
%     likeinds = LI{i};
