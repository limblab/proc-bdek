function[best_PDS,preferred_epoch,alltuned,fulldat] = tuning_types_PD_func(alldays,brain_area,CO_index,dir_column,loop_alignmentsPD,loop_rangesPD)
...
for loopthrough = 1:length(loop_alignmentsPD)
    ...
    for i = 1:length(co_units) % Loop through units
        ...
        [PD,PD_bounds,PD_dist,MAG_dist,curve_params] = PD_tuning_VM(co_units,alldays(CO_index).tt(:,dir_column),...
