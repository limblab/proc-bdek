function[pfits,thetas,curve,base,gain,fwhm] = VM_fit_constrained(x,y,theta_list,varargin)
...
b_i = [binit1, gain_i, k_i];
b_i2 = [binit2 , -gain_i, k_i];
...
vm_func  = @(xs,p) p(1) + p(2)*exp(p(3)*cos(xs));
...
[pfits1,~,exitflag] = fmincon(costfunc,b_i,[],[],[],[],bounds_L,bounds_H,@(x)nonlin_helper(x),fmopts);

ypreds = reshape(vm_func(x,pfits1),[],1);
