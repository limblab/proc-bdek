...
loop_alignments = {6};%,'go'};
loop_ranges = {[300 1000]};%,[50 250]};
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
baseline_col = 6;
% fire_align = {7};
% fire_ranges = {-500:100:1000};
% fire_ranges = { -225:25:1000 , -25:25:425}; 
% fire_ranges = {[-200:50:1000],[0:50:350],[0:50:1000],[-100:50:200]};
fire_ranges = {[-200:50:1000]};%,[0:10:350],[0:10:1000],[-100:10:200]};
% fire_ranges = {-100:50:1000,0:50:350,0:50:1000,0:50:200};%{-100:25:700, 0:25:400};%{0:25:700};% , -100:50:500}; 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
FR_thresh = 1;
plotday = 2;
CO_index = 1;
...
%     [~,~,~,fulldatPDS] = tuning_types_PD_func(alldays,brain_area,CO_index,reachdir_col,loop_alignments,loop_ranges);
%     best_PDS = get_PDS_fulldat(fulldatPDS,pi/2);
...
for i = 1:length(co_units) % Loop through units
    ...
    [tbt_rast,tbt_inds] = raster_plot(co_units{i},alldays(plotday).tt,[tpre1 tpre2]./1000,baseline_col,0,'none');
...
%% DO Peak fitting
if do_peak_fit
    [decode_dir,boot_decode,decode_bounds,avs_in_bins,decode_unc,decode_std,perform] = deal(cell(length(FRS),1));
    figure; hold on; 
    clrs = {'b','r','c','m'};
...
%         decode_std{i}(1,:) = circ_std(decode_dir{i});
        perform{i}(1,:) = 1-circ_var(decode_dir{i});
%         [decode_std{i}(2,:),decode_std{i}(3,:)] = boot_bounds(1000,@circ_std,decode_dir{i},2.5,97.5);
        plot(time_cents,perform{i}(1,:),clrs{i});
