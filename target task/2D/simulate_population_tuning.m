% This is a simulation exercise to see if various tuning curve changes in
% single neurons can be detected using population "readouts"
...
T = 1000;    % Trial duration in milliseconds
...
baseshift2 = 5*ones(N,1); %(1/5)*gamrnd(5,1,N,1);        % the baseline gets shifted by this factor in Condition 2 (LOW)
baseshift3 = 20*ones(N,1); %gamrnd(5,1,N,1);        %0*rand(N,1);      % the baseline gets shifted by this factor in Condition 3 (HIGH)

gainscale2 = ones(N,1); %0.1*gamrnd(5,1,N,1);%0.5*rand(N,1);     % the gain gets scaled by this factor in Condition 2 (LOW)
gainscale3 = ones(N,1); %0.02*gamrnd(5,1,N,1);%0.1*rand(N,1);         % the gain gets scaled by this factor in Condition 3 (HIGH)
...
for n=1:N
    ...
    for tr = 1:size(alltts,1)
        ...
        if alltts(tr,3)==10000
            lam = base(n) + gain(n)*cos(alltts(tr,10) - theta0(n));
        elseif alltts(tr,3)==50
            lam = baseshift2(n) + base(n) + gainscale2(n)*gain(n)*cos(alltts(tr,10) - theta0(n));
        elseif alltts(tr,3)==5
            lam = baseshift3(n) + base(n) + gainscale3(n)*gain(n)*cos(alltts(tr,10) - theta0(n));
        end
        %lamvec = lam.*tgain.*tcs{tctype(n)};
