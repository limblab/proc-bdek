...
typ = zeros(length(modrange),3);
for neurnum = 1:length(modrange)
    
    maxresponse = [modrangeH{neurnum}(1,:); modrangeL{neurnum}(1,:)];
    minresponse = [modrangeH{neurnum}(2,:); modrangeL{neurnum}(2,:)];
    ranges = modrange{neurnum}(1,:) - modrange{neurnum}(2,:);
    rangesMAX = modrangeH{neurnum}(1,:) - modrangeL{neurnum}(2,:);
    
    tc2 = neurons{2}{neurnum}.tuning;
    tc2_L = neurons{2}{neurnum}.tuning_low;
    tc2_H = neurons{2}{neurnum}.tuning_high;
    
    tc3 = neurons{3}{neurnum}.tuning;
    tc3_L = neurons{3}{neurnum}.tuning_low;
    tc3_H = neurons{3}{neurnum}.tuning_high;
    
    tc4 = neurons{4}{neurnum}.tuning;
    tc4_L = neurons{4}{neurnum}.tuning_low;
    tc4_H = neurons{4}{neurnum}.tuning_high;
      
    %vis(neurnum) = max(tc2) > maxresponse(1,1) | min(tc2) < minresponse(2,1); 
    vis(neurnum) = maxresponse(2,2) > maxresponse(1,1) | minresponse(1,2) < minresponse(2,1);
    %vis(neurnum) = max(tc5) > min(tc5_H) & min(tc5) < max(tc5_L);
    ebu(neurnum) = max(tc3) > min(tc3_H) & min(tc3) < max(tc3_L);
    lbu(neurnum) = max(tc4) > min(tc4_H) & min(tc4) < max(tc4_L);
    
    maxtcs = modrange{neurnum}(1,2:end);
    
    typ(neurnum,:) = find(maxtcs == max(maxtcs));
    if vis(neurnum) + ebu(neurnum) + lbu(neurnum) == 0
        typ(neurnum,:) = 0;
    end
   
    
end
neur_type = unique(typ','rows')';
...
both_build = find(ebu+lbu == 2);
vis_plus_build = any_build(ismember(any_build,with_vis));
novis_plus_build = any_build(ismember(any_build,without_vis));

vis_plus_ebuild = early_build(ismember(early_build,with_vis));
vis_plus_lbuild = late_build(ismember(late_build,with_vis));
vis_plus_bothbuild = both_build(ismember(both_build,with_vis));
...
novis_plus_bothbuild = both_build(ismember(both_build,without_vis));

typevis = find(neur_type==1);
typebue = find(neur_type==2);
typebul = find(neur_type==3);

buildup = unique([typebue; typebul]);
%%
...
%%
good_neurs = PMd.typevis;
%good_neurs = early_build;
%good_neurs = without_vis;
 
%shuff = randperm(length(novis_plus_build));
%good_neurs = novis_plus_build(shuff(1:length(with_vis)));
...
%                 expect_counts{z}{bin}(i,q) = sum(high_counts);
        ...
        firing_perc{z}{bin} = (trial_counts{z}{bin}(:,good_neurs) - repmat(min(tuning_array(good_neurs,:),[],2)',...
            length(tt_pred),1))./moduls{z}(:,good_neurs);
