...
%     alldays(task_block).tt(:,11) = alldays(task_block).RT; %#ok<SAGROW>
%     badtrls = find(isnan(alldays(task_block).tt(:,11)));
%     alldays(task_block).tt(badtrls,11) = alldays(task_block).tt(badtrls,7);
% end   

areas = {'PMd','M1'};%,'M1'};%,'M1'};%,'M1'};%{'PMd','M1'};
NBOOT = 1;
for ba_loop = 1:length(areas)
    ...
    filttype = 'causal';
    align_col = 7; % 17 for chosen, 13 for correct

    t1_t2 = {[4 -200],[5 200]}; % [column time_offset(ms)]
...
%               [6     0] ,  [7   0] };%
    ...
    LENS = max(cell2mat(cellfun(@(Y) cellfun(@(x) min(diff(x,[],2)),Y),INDS,'UniformOutput',0)),[],1);
    CLENS = [1 cumsum(LENS)];
    ...
    execution_index = autosplit{sum(cell2mat(cellfun(@(x) x==[5 0],alcols(:,1),'Uni',0)),2)==2}(1);
...
%                 DR_ball{tlc,1} = cell2mat(DR_b(tlc,mem)');
            ...
            for t = 1:T
                ...
                if bli == Baseline_block
                    for i = 1:size(NTR,1)
                        DR_b2 = DR_b;
                        matchtrl = sum(DR_b{abs(targids{bli}(i)),t},2)==sum(NTR(i,:,t),2);
                        if sum(matchtrl)>0
                            DR_b2{targids{bli}(i),t}(matchtrl,:) = [];
                        end
                        if sum(cellfun(@(x) size(x,1),DR_b2(:,t))<NDIM)>0 % Too few training trials
