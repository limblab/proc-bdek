function[raster,allinds,fig_hand,SpikeTime] = raster_plotCK(train,trial_table,t1_t2,alignment,do_plot,position_rank,TitleString,sort_column,varargin)
...
if isempty(sort_col);
    addcol = size(trial_table,2) + 1;
    trial_table(:,addcol) = 1;
    sort_col = addcol;
end
...
all_likes = unique(trial_table(:,sort_col));
...
for i = 1:length(all_likes)
    allinds{i} = find(trial_table(:,sort_col)==all_likes(i));
...
for q = 1:length(all_likes)
    ...
    for i = 1:LA{q}
        ...
        if rank_anchor == 1
            final_pos{q}(i) = allvartrials{q}(i,18);
...
%% Do Plots
    if do_plot ~= 0
        ...
        plot(xplotter,raster{q},PlotColors{q},'MarkerSize',5);
    end
end
