...
lowinds = find(trial_table(:,3)==min(trial_table(:,3)));
highinds = find(trial_table(:,3)==max(trial_table(:,3)));
...
for i = 1:LL
    ...
    if rank_anchor == 1
        final_pos = bdf.pos(bdfrew,2);
    elseif rank_anchor == 2
        final_pos = lowvartrials(i,3);
