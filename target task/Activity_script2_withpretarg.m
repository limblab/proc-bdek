...
for ranges = 1:length(tune_ranges)-1
    ...
    for i = 1:length(co_units) % Loop through units
        ...
        if t1 >= 0 && t2 >= 0
            ...
            alldays(CO_index).tt,t1,t2,tune_align,i,1000);
        else
...
for bin = 1:length(time_bins) - 1
    ...
    %     errorbar(1:length(set_of_inds),100*horzcat(mk{z}{:}),...
    %         100*(horzcat(mk{z}{:})-horzcat(lk{z}{:})),100*(horzcat(uk{z}{:})-horzcat(mk{z}{:})),colors_of_plot{z});
    %     
    %     leg_ent{z} = sprintf('K (prior dist): %d',priors{prediction_day_indices(z)}.val);
    end
end

% title(sprintf('%s (%s-%s-%s)',brain_area,MO,DA,YE),'FontSize',16);
% xlabel('Cue Concentration (kappa)','FontSize',14);
% ylabel('FR change (% modulation)','FontSize',14);
% set(gca,'xtick',1:length(set_of_inds));
% set(gca,'xticklabel',mat2cell(num2str(set_of_inds),ones(1,length(set_of_inds))));
% legend(leg_ent);
% 
% ylim([-20 100]);
%%
...
%%
% bothtt = vertcat(c_tt{prediction_day_indices});
% 
% allind = cell(length(set_of_inds),1);
% for qq = 1:length(set_of_inds)
%     allind{qq} = find(bothtt(:,3)==set_of_inds(qq));
% end
% 
% bothsess1 = vertcat(firing_diffs{1}{1},firing_diffs{2}{1});
% bothsess2 = vertcat(firing_diffs{1}{2},firing_diffs{2}{2});
% 
% avdiff1 = mean(bothsess1,2);
% avdiff2 = mean(bothsess2,2);
% 
% avstd1 = std(bothsess1,[],2);
% avstd2 = std(bothsess2,[],2);
% 
% binavm1 = bin_array(avdiff1,12,1);
% binavm2 = bin_array(avdiff2,12,1);
% 
% binstd1 = bin_array(avstd1,12,1);
% binstd2 = bin_array(avstd2,12,1);
% 
% figure; hold on; 
% subplot(1,2,1); hold on;
% plot([binavm1,binavm2]);
% plot(repmat(12*(length(alldays(2).tt)/length(bothsess1)),1,2),[-.5 1],'k--');
% 
% subplot(1,2,2); hold on;
% plot(repmat(10*(length(alldays(2).tt)/length(bothsess1)),1,2),[3 6],'k--');
% plot([binstd1,binstd2]);
% 
